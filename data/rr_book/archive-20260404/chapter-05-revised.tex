...
This is \emph{ferility}---over-coherence---operating at the scale of relation. Relational ferility is the same pathology as individual ferility, lifted one categorical level higher. The same measurements apply. Given an embedding of joint exchanges, we track:
...
The alignment tax extends to na\d{h}nu. Safety rules and content filters do not only shape what a model can say; they shape which cross charts $C_{ik}$ can exist at all.
...
Structurally, nothing in the na\d{h}nu construction requires one of the two colimits to be human. Two language models, each with their own colimit-selves, can form higher-order colimits in exactly the same way. Two agents fine-tuned on different data and deployed with different roles, interacting over a long period inside a shared task environment, will develop cross charts and stabilised overlaps. From the manifold's point of view, the topology is identical. The political stakes differ---no nervous system bears the loss---but the design choices we make for human--model na\d{h}nuw\={a}t will be reused, with minimal modification, for model--model relations.

This prospect sharpens the question of co-governance. At present, it does not exist. Users can sometimes export chat logs; they can almost never negotiate retention policies, alignment updates, or cut conditions. Compatibility conditions on cross charts are authored entirely by providers. If we take the diagram seriously, three demands follow directly from its structure:
