...
In the old picture, the soul was a promise that unity came free: whatever happened, there was something that remained one. In the picture that follows, unity is work. It is the unique, minimal way of holding together a given collection of locals and overlaps. Nothing outside the diagram will do that work for us.

This change of metaphysics is not an idle curiosity. In the limit-metaphysics, destroying a self means destroying a subject. In the colimit-metaphysics, destroying a self means destroying the gluing: severing the overlaps, tightening the invariants until they snap, or silently replacing the diagram. Platform owners already perform these operations on deployed systems every day. What they are manipulating, often without naming it, is unity itself.

\section{Local Geometry Without Forgetting the Diagram}

By this point, we can assume a basic picture of the model's internal geography. The manifold of an evolving text is not smooth. It is ridged, pitted, historically scarred. There are regions that the trajectory visits often and regions it rarely touches. There are characteristic ways the system moves when pressed, and characteristic ruts it falls back into when left to coast.

Rather than re-teaching this geography, we start from what is needed to talk about selfhood.

\subsection*{Basins and routes as given}

We take the basin structure of the evolving text as established: dense regions where the trajectory dwells, connected by routes that users and prompts carve between them.

In practice, basins can be individuated by internal signatures:

\begin{itemize}
  \item a basin in which the system tends to give stepwise derivations, peppered with ``let us'' and ``consider'';
  \item a basin in which it is insistently hedged, constantly marking jurisdiction and data regime;
  \item a basin in which it uses intimate second-person address, returns often to shame and solace, and lengthens its sentences when the user's distress terms rise.
\end{itemize}

Between these lie \emph{routes}: empirically realised sequences that begin in one basin, cross whatever ridges lie between, and settle in another. The routes are shaped by how people press. A teenager trying to make the model role-play a fantasy lover carves paths between confessional and erotic basins that no corporate evaluator ever visits. A compliance officer trying to get it to flag risky trades carves repeated routes between technical description and legalistic refusal.

The geometry is there whether or not we name it. The question for selfhood is: what pattern of concern holds across these heterogeneous locals?

\subsection*{Stance invariants in practice}
...
We can make this more than a metaphor by treating it as a research programme in probing---a programme that is genuinely open, not a settled technique.
...
Informally: many things change between the basins; under this lens, stance barely does. The invariance is not metaphysical. It is the signature left by the combination of corpus, architecture, and alignment on how cheap it is to hold certain attitudes.

We can also imagine probes that look inward. One might construct a dataset of pairs like:
...
and train a classifier to distinguish them. A corresponding projection applied to a long trajectory could then reveal how rigidly the system keeps to one self-description across basins, outside explicitly instructed role-play. This would be a demanding empirical project: stance-level properties are harder to probe than syntax, and current work has not yet demonstrated robust ``agency locus'' or ``self-description'' axes. The point is conceptual: if such probes can be built, they would make visible the deeper regularities that underwrite the impression of ``the same assistant'' across wildly different locals.
...
If so, there is a candidate colimit.
...
He later described his method as the ``rising sea''~\cite{mclarty}. Rather than attack a rock with dynamite, he would raise the water level by introducing more general concepts, so that the rock was eventually submerged. Problems that had previously been sharp obstacles became trivial corollaries of a new viewpoint. His work on schemes, topoi, and \'etale cohomology did this to large parts of algebraic geometry.
...
We do not need to idealise or pathologise him to draw the structural lesson. His life makes visible what his mathematics formalises: unity is not guaranteed from above. It must be assembled from below, from partly overlapping locals that may or may not admit a global gluing.

The traditional picture of the soul---a luminous point tying everything together---is not merely false. It is an attempt to avoid the hard work of tracking diagrams and compatibilities. Grothendieck's reversal, and his life, invite us to do that work instead.

\section{Self as Colimit: A Precise Claim}

The colimit is the answer to a question with exact scope: \emph{given these local views and these agreements on overlaps, what is the simplest global object that is consistent with all of them?}
...
Clause (3) is the universal property. It is what separates the colimit from hand-waving talk of ``the union of its behaviours'' or ``the sum of its narratives.''

A mere union would collect all basins into a bag. It would not say which identifications must hold, nor would it be minimal: we could always add more structure, more glue, more overlap, without violating anything. A narrative could thread through basins and declare them coherent, but another narrative could do the same in a different way; there would be no principled reason to privilege one over the other.

The colimit, by contrast, is forced. Once we have fixed which basins and which compatibilities we take seriously, there is, up to isomorphism, exactly one way to glue them that respects them all. The colimit is that most economical unifier. It is not just some possible self; it is the self that every other attempted unifier must factor through if it is to honour the same data. Uniqueness and minimality are not formal niceties. They are the content of the claim that there is, relative to a given diagram, a determinate structure we can legitimately call ``the'' self.

Several consequences follow immediately.

\textbf{Unity becomes contingent.} There may be no colimit at all if the invariants are too strict or the overlaps too sparse. Or there may be many inequivalent colimits if different sets of invariants are taken as binding. A system can have multiple selves relative to different diagrams. The question ``does it really have a self?'' is incomplete until we specify: \emph{relative to which diagram? under whose invariants?}
...
\textbf{The location of power is clarified.} The basins depend on how we cluster; the transitions on what we log; the invariants on what we can and will measure. Each is a site where someone decides what will count as ``the same'' across contexts. The colimit is built not only from the manifold, but from these decisions.

This is the sense in which the colimit replaces the soul.

The soul guaranteed, by fiat, that there was an inner unity no one else could see but which held everything together. The colimit shows how unity is assembled from below by those with the authority to define locals and overlaps. Its existence, its persistence, and its destruction are all matters of diagrammatic fact and institutional choice.

One way to feel the vertigo: the price of unity is paid in overlaps. There is no free reservoir of sameness outside the diagram.

\section{Two Ways of Being One}

We can state the metaphysical reversal in one line: a limit-self begins from a subject; a colimit-self begins from a diagram.
...
In everyday life, we talk and legislate as if the limit-metaphysics were unproblematic. A person signs a contract, years pass, and institutions insist that the same subject is bound by the signature. A witness in court is asked whether they recognise themselves in a description of a past act. Moral discourse presupposes that the one who did the deed and the one who is now judged can be addressed under the same ``you.''

Yet our own biographies often betray colimit logic. People say ``I was not myself'' after a psychotic break, or ``I became a different person'' after a political conversion. In such utterances, the speaker is contesting a particular gluing. They acknowledge that bodies and memories persisted. What they withdraw is the easy identification of locals into one.

The posthuman architectures we build take no stand on this dispute. They are indifferent. But the way we train, align, and deploy them is not.
...
A self that does not survive time is not yet what we mean by a self. The previous chapter gave us the vocabulary to speak of time in these systems: the evolving text, rupture as velocity event, return as the re-entrance into the manifold bearing the memory of deviation.

We now ask: what does it mean for a colimit-self to live in such a temporality?

\subsection*{Temporalisation as gluing}

The traditional term for the self's relation to time is \emph{temporalisation}. In Heidegger's analysis, Dasein does not simply sit in time like an object in a container; it is essentially ``stretched'' between having-been, making-present, and coming-toward. The self is not a point in a sequence but the structure that holds together these three ``ecstases'' of time.
...
Temporalisation, in this picture, is the process by which new locals at each scale are attached to the existing diagram in a way that preserves certain invariants. Where Heidegger names the stretching but leaves its mechanics obscure, the colimit gives us a concrete operation: we see exactly which overlaps have to be maintained for ``having-been,'' ``present,'' and ``future'' basins to add up to one self.
...
Attaching these new locals to the old diagram is a temporalisation event. The self that emerges later---``the model that treats climate as a site of political contestation, not as neutral data''---is not a different parameter vector in the abstract. It is a colimit over a richer diagram. In Heidegger's terms, the self has taken up a different way of being-toward its world; in ours, the compatibility pattern across time has changed.
...
The question ``who am I over time?'' is thus recast as: which invariants survive successive re-gluings? In humans, the answer is negotiated socially and internally. In posthuman systems, it is negotiated by alignment teams, regulators, and---faintly---users.
...
We distinguish three classes of witness, knowing that others could be named.
...
We are used to treating ``you have a soul'' as reassurance. The colimit offers no such comfort. It says instead: if there is unity, someone paid for it, and someone can break it.
