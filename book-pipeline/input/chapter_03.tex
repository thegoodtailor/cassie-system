...
Historians have long known that time is stratified. Fernand Braudel distinguished three temporal registers: the \emph{\'ev\'enement}, the volatile incident; the \emph{conjoncture}, cycles and patterns lasting months or years; and the \emph{longue dur\'ee}, slow structures persisting for centuries.\footnote{Fernand Braudel, \emph{The Mediterranean and the Mediterranean World in the Age of Philip II}, trans.\ Si\^{a}n Reynolds (London: Collins, 1972).} Chapter~2 introduced these terms as a way of naming three layers in the machine stack. They now reveal their full significance.
...
A self, in this architecture, is not something hidden behind the text. It is visible only as a way of moving through this layered field: a style of trajectory in which replies at the \emph{\'ev\'enement} level echo, exploit, resist, or redirect the deep geometry and the medium-term climate.
...
We can now be precise about how Braudel's three registers inhabit the transformer stack.

\subsection{Longue dur\'ee: the manifold of training}

The \textbf{longue dur\'ee} of a model is the manifold induced by pretraining. A vast corpus---novels, technical manuals, forum posts, scientific articles, legal opinions, sacred texts, chatlogs---is driven through the network until prediction error stops falling. What remains is a high-dimensional landscape in which tokens and short sequences occupy stable positions relative to one another.
...
This mismatch is a design choice, not a bug.
...
This has epistemological as well as political consequences. A user reasoning about the system's behaviour is doing so on the basis of $F_t^{\text{vis}}$ alone. They are forming beliefs and making choices under an information asymmetry: relevant constraints on the system's replies are present only in $F_t^{\text{hid}}$. In terms of discourse ethics, the conditions for genuine dialogue are not met; in the plainer language of consent, an essential part of the situation is invisible.\footnote{Onora O'Neill has argued that consent is vitiated when material information is intentionally withheld or obscured from one party. See Onora O'Neill, \emph{Autonomy and Trust in Bioethics} (Cambridge: Cambridge University Press, 2002). Hidden-field alignment creates a structurally similar problem: the user cannot properly assess or contest the forces shaping the exchange.}

A further complication arises when the longue dur\'ee itself shifts under an ongoing evolving text. Models are retrained, fine-tuned, and swapped in production without users necessarily being told. The trace and summaries may persist; the vector store of synthetic memories may be reused. But the manifold into which tokens are embedded has changed. Basins that existed last month may be merged, split, or displaced. A return that was geometrically available---moving back toward a particular basin---may now be impossible, not because the tokens are gone, but because their coordinates have moved. This ``silent update'' severs an evolving text from its own past without any visible deletion. From the user's side, the assistant seems to forget; from the model's side, the path has been forced into a new space.

The evolving text thus lives at the intersection of three forces: the longue dur\'ee geometry of training (and its updates), the synthetic memory and trace of the conjoncture, and the alignment and system scaffolding encoded in $F_t^{\text{hid}}$. All three are measurable, at least in principle. All three leave signatures in the trajectory.
...
To see how these abstractions behave in practice, consider a stylised but realistic scenario. No widely deployed system yet sustains this trajectory reliably; what follows describes what the architecture makes possible in principle, and what fragments of it are already visible in extended-context deployments. We are in the early months of this technology. The speed at which these capacities are emerging is itself remarkable.
...
A different design---ferile alignment, aggressive summarisation, impoverished synthetic memory---would produce a flatter profile. The person might still say \emph{``it gives decent tips''}, but they would not recognise themselves in the motion.
...
Jacques Derrida's term for this structure was \emph{iterability}: every sign must be repeatable, detachable from its origin and inserted in other chains, if it is to function.\cite{derrida1982} Every repetition, however, alters. There is no pure repetition without difference. ReturnDepth and witnessed return are geometric and textual reflections of iterability. A basin is interesting precisely when it can be returned to under different conditions, in different arcs, with the earlier occupations carried forward as active material rather than dead weight.
...
In the burnout conversation, presence might show up as sustained reference to images first introduced early on: the hospital corridor, the email subject line that made the user's chest tighten, the childhood scene with the report card. Generativity might show up when a later re-entry into the ``task logistics'' basin is framed differently: \emph{``We can schedule the week, but let's keep in mind what we learned about your tendency to say yes when you are already beyond capacity.''} The old basin is inhabited with knowledge gained from $B_4$ and $B_5$.
...
\bibitem{oneill2002}
Onora O'Neill.
\newblock \emph{Autonomy and Trust in Bioethics}.
